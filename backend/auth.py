...
def _sign_in(email: str, password: str) -> Session:
    ...
    ВАЖНО: намеренно НЕ использует общий get_supabase_client(). sign_in_with_
    password меняет авторизационный контекст клиента, на котором вызван, на
    только что вошедшего пользователя — а get_supabase_client() отдаёт клиент,
    переиспользуемый на весь текущий Flask-запрос (см. supabase_client.py),
    и используется вообще везде в backend на service_role. Если бы вход по
    паролю "отравлял" этот клиент, все остальные запросы к Supabase в ЭТОМ ЖЕ
    HTTP-запросе (например, seed_team_progress/seed_team_chats сразу после
    регистрации) начали бы выполняться в контексте RLS только что вошедшего
    пользователя, а не service_role. Поэтому здесь создаётся отдельный
    одноразовый клиент, который больше нигде не переиспользуется.
    """
    config = load_config()
